...
route.post(`/add`, async (req, res) => {
    try {
        ...
        if (!name || !price) return res.status(401).json({ error: 'Error adding products' });
...
route.post(`/cart-add`, async (req, res) => {
    try {
        ...
        if (!productId || !userId) return res.status(401).json({ error: 'Error adding to cart...' });
...
route.get(`/get-cart/:userId`, async (req, res) => {
    try {
        ...
        if (cartIds.length === 0) return res.status(200).json({ message: 'Cart fetched', cart: [] });
